...
class FornecedorUser(HttpUser):
    ...
    def fluxo_fornecedor(self):
        ...
        # Gera um CNPJ único de 14 dígitos usando timestamp de microssegundos
        # O prefixo '99' ajuda a identificar que são dados de teste
        ...
        with self.client.post("/api/fornecedores", json=payload_fornecedor, headers=headers, catch_response=True) as response:
            if response.status_code in [200, 201]:
                try:
                    # Tenta capturar o ID (verifique se na sua API é 'id' ou 'idFornecedor')
