...
class CargoUser(HttpUser):
    ...
    def fluxo_cargo(self):
        ...
        # Gerar um sufixo único para o nome do cargo para aguentar o teste de carga
        # Usamos uma parte de um UUID ou timestamp para garantir que não repita
        ...
        # --- PASSO 1: Criar o Cargo (POST) ---
        with self.client.post("/api/cargos", json=payload_cargo, headers=headers, catch_response=True) as response:
            if response.status_code in [200, 201]:
                try:
                    # Tenta capturar o ID do cargo recém-criado
                    # Ajuste 'id' para 'codigo' ou 'idCargo' se sua API usar outro nome
                    cargo_id = response.json().get("id") or response.json().get("idCargo")
                    
                    if cargo_id:
                        # --- PASSO 2: Ler o Cargo Criado (GET) ---
                        # O parâmetro 'name' agrupa as URLs diferentes no relatório do Locust
